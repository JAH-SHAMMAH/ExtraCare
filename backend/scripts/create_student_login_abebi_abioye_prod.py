...
async def main():
    if len(sys.argv) < 2:
        print("usage: create_student_login_abebi_abioye_prod.py '<postgresql_url>'")
        return 2
    ...
    try:
        ...
        existing_user = await conn.fetchval(
            "SELECT id FROM users WHERE id = (SELECT user_id FROM students WHERE id = $1)", student_id
        )
        if existing_user:
            print("[ERROR] Student already has a user account linked")
            return 1
