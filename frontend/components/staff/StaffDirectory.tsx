...
/**
 * Shared staff & admin directory. Rendered by BOTH the school module's Staff page
 * and the HR Manager › PIM › Employee List page, so the HR tab-bar shell stays
 * intact under /dashboard/hrm. The optional `breadcrumb` tailors the header to
 * whichever section is hosting it.
 */
export function StaffDirectory({ breadcrumb = ["People & HR", "Staff & Admin"] }: { breadcrumb?: [string, string] }) {
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <div className="mb-6">
        <nav className="flex items-center gap-2 text-xs text-slate-400 mb-2"><span>{breadcrumb[0]}</span><span>/</span><span className="text-brand-600 font-semibold">{breadcrumb[1]}</span></nav>
