...
import {
  BookOpen, BookPlus, Search, X, Loader2, Check, AlertTriangle,
  ArrowRight, Filter, User as UserIcon, Clock, RotateCcw, Trash2,
} from "lucide-react";
import {
  useLibraryBooks, useLibraryLoans, useLibraryStats,
  useCreateBook, useDeleteBook, useIssueLoan, useReturnLoan,
    ...
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        ...
        <StatCard
          label="Overdue"
          value={stats?.overdue ?? "—"}
          color={stats?.overdue ? "bg-rose-50 text-rose-700" : "bg-slate-50 text-slate-500"}
          icon={AlertTriangle}
...
function CatalogueTab({ onIssue }: { onIssue: (book: LibraryBookRow) => void }) {
  const [search, setSearch] = useState("");
  const [category, setCategory] = useState<string | undefined>(undefined);
  const [availableOnly, setAvailableOnly] = useState(false);

  const { data, isLoading } = useLibraryBooks({
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {books.map((b) => (
            <BookCard
              ...
              onIssue={() => onIssue(b)}
              onDelete={() => {
                if (confirm(`Remove "${b.title}" from the catalogue?`)) {
...
function BookCard({
  book, onIssue, onDelete,
}: { book: LibraryBookRow; onIssue: () => void; onDelete: () => void }) {
  const available = book.available_copies > 0;
  return (
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden hover:shadow-md transition-shadow group">
      <div className={cn(
        "h-24 flex items-center justify-center border-b",
        available ? "bg-gradient-to-br from-brand-50 to-indigo-50 border-brand-100" : "bg-slate-50 border-slate-100",
      )}>
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-2 mb-2">
          {book.category && (
            <span className="badge bg-slate-100 text-slate-600 border-slate-200 text-[9px]">
              {book.category}
            </span>
          )}
          <span className={cn(
        ...
        <p className="text-[10px] text-slate-400 mt-1">
          {[book.publication_year, book.shelf_location ? `Shelf ${book.shelf_location}` : null].filter(Boolean).join(" · ")}
        </p>
        <div className="flex gap-2 mt-3">
          <button
            ...
            disabled={!available}
            className={cn(
              "flex-1 btn-primary text-xs py-1.5",
...
function LoansTab() {
  ...
  const { data, isLoading } = useLibraryLoans({
    ...
    <>
      <div className="bg-white rounded-xl border border-slate-200 p-1 mb-4 inline-flex">
        {[
          ...
          { id: "borrowed" as const, label: "Active" },
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50/80 border-b border-slate-100">
                {["Book", "Borrower", "Issued", "Due", "Status", ""].map((h) => (
                  <th key={h} className="px-5 py-3 text-[10px] font-bold uppercase tracking-widest text-slate-500">
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-slate-50">
              {loans.map((l) => <LoanRow key={l.id} loan={l} onReturn={() => returnLoan.mutate(l.id)} />)}
            </tbody>
          </table>
        </div>
      )}
  ...
  return (
    <tr className={cn("hover:bg-slate-50/60 transition-colors", loan.is_overdue && "bg-rose-50/30")}>
      <td className="px-5 py-3.5">
        <p className="text-sm font-bold text-slate-900 line-clamp-1">{loan.book_title ?? "—"}</p>
        <p className="text-xs text-slate-500">{loan.book_author ?? ""}</p>
      </td>
      <td className="px-5 py-3.5">
        <p className="text-sm font-medium text-slate-800 line-clamp-1">{loan.borrower_name ?? "—"}</p>
        <p className="text-xs text-slate-500 line-clamp-1">{loan.borrower_email}</p>
      </td>
      ...
      <td className="px-5 py-3.5 text-xs tabular-nums">
        <span className={cn(
          loan.is_overdue ? "text-rose-700 font-bold" : "text-slate-600",
        )}>
          {loan.due_date}
        </span>
      </td>
      <td className="px-5 py-3.5">
        {returned ? (
          <span className="badge bg-slate-100 text-slate-600 border-slate-200 text-[10px]">Returned</span>
        ) : loan.is_overdue ? (
...
function AddBookDrawer({ onClose }: { onClose: () => void }) {
  const [form, setForm] = useState({
    title: "", author: "", category: "", publisher: "",
    publication_year: "", isbn: "", shelf_location: "",
  ...
  return (
    <DrawerShell title="Add a book" subtitle="Catalogue a new title" onClose={onClose}>
      <div className="space-y-4 p-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} placeholder="Fiction, Science…" className="input" />
          </div>
          <div>
            <label className="label">Shelf</label>
            <input value={form.shelf_location} onChange={(e) => setForm({ ...form, shelf_location: e.target.value })} placeholder="A1-12" className="input" />
          </div>
          <div>
            <label className="label">Total copies</label>
            <input type="number" min={1} value={form.total_copies} onChange={(e) => setForm({ ...form, total_copies: Number(e.target.value) })} className="input" />
          </div>
          <div>
            <label className="label">Year</label>
            <input type="number" min={1800} max={2100} value={form.publication_year} onChange={(e) => setForm({ ...form, publication_year: e.target.value })} className="input" />
          </div>
      ...
      <div className="sticky bottom-0 bg-white border-t border-slate-200 px-6 py-4 flex items-center justify-end gap-2">
        ...
        <button
          onClick={save}
          disabled={create.isPending || !form.title.trim() || !form.author.trim()}
...
function IssueLoanDrawer({ book, onClose }: { book: LibraryBookRow; onClose: () => void }) {
  ...
  const matches = ((students?.items ?? []) as Array<{
    id: string; first_name: string; last_name: string; user_id?: string | null; student_id: string;
  }>).filter((s) => !!s.user_id);
  ...
  return (
    <DrawerShell title="Issue book" subtitle={book.title} onClose={onClose}>
      <div className="p-6 space-y-5">
        ...
        <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 border border-slate-100">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-slate-500 truncate">{book.author} · {book.available_copies} of {book.total_copies} available</p>
          </div>
        </div>
        ...
        <div>
          ...
          {borrower ? (
            <div className="flex items-center gap-2 p-2 rounded-lg border border-brand-200 bg-brand-50">
              ...
              <span className="text-sm font-semibold text-brand-800 flex-1">{borrower.label}</span>
              <button onClick={() => setBorrower(null)} className="text-brand-600 hover:text-brand-800">
                <X size={14} />
              </button>
            </div>
          ) : (
            <>
              ...
              {search && matches.length === 0 && (
                <p className="text-xs text-slate-400 mt-1">No students match.</p>
              )}
            </>
          )}
        </div>
      ...
      <div className="sticky bottom-0 bg-white border-t border-slate-200 px-6 py-4 flex items-center justify-end gap-2">
        ...
        <button
          onClick={submit}
          disabled={!borrower || issue.isPending}
          className="btn-primary gap-2"
        >
          {issue.isPending ? <Loader2 size={14} className="animate-spin" /> : <ArrowRight size={14} />}
          Issue book
        </button>
      </div>
    </DrawerShell>
      ...
      <div className="relative ml-auto w-full max-w-lg bg-white shadow-2xl overflow-y-auto animate-slide-in flex flex-col">
        <div className="sticky top-0 z-10 bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between">
          <div className="min-w-0">
            ...
            {subtitle && <h2 className="text-lg font-black text-slate-900 truncate">{subtitle}</h2>}
          </div>
